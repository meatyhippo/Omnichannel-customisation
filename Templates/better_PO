...
{% set show_EAN = false %} {# shows EAN #}
{% set show_custom_SKU = false %} {# shows SKU #}
{% set show_manufacturer_SKU = false %} {# shows SKU #}
{% set show_item_barcode = false %} {# shows the sys id of the item on the line for easy adding to inventory #}
{#
***End Custom Options***
#}
...
<div class="startStop">
Ordered: {{Order.orderedDate|correcttimezone|date ("d/m/y")}}<br />
Expected: {{Order.arrivalDate|correcttimezone|date ("d/m/y")}}<br /> 
</div>
</header>
<div class="row">
<div class="col-md-6">
...
{% endif %}
{% if show_custom_SKU == true %}
<td>{{OrderLine.Item.customSku}}</td>
{% endif %}
<td><p style="float: left;">{{OrderLine.Item.description}}</p>{% if show_item_barcode == true %}<img  height="40" width="200" src="/barcode.php?type=label&number={{ OrderLine.Item.systemSku }}&noframe=1" style="margin: auto 0;display: block;float: right;">{% endif %}</td>
