...
{# Customer Account #}
{% set show_credit_account_signature = true %}     {# Prints Store Copy with signature line on accounts that use an Account Credit (not Deposit) #}
...
.barcodeContainer { page-break-after: always; }
...
{% endif %}
		...
		{% if not parameters.page or parameters.page == 2 or not page_loaded %}
			...
			<div>
				...
				{{ _self.title(Sale,parameters,_context,merchantcopy) }}
				{{ _self.date(Sale) }}
...
{% macro title(Sale,parameters,options,merchantcopy) %}
		...
		{% if Sale.calcTotal >= 0 %}
			{% if Sale.completed == 'true' %}
				...
				{% if options.workorders_as_title and Sale.SaleLines is empty and Sale.Customer.Workorders is defined %}
					 Work Orders
				{% else %}
{% if merchantcopy == 'true' %} Merchant copy{% else %}
