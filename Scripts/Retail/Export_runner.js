...
// add new buttons here
	(()=>{
		// n_('name', 'endpoint / type', 'load relations (value or 0)', 'query', 'description', UI location, public release?)
		n_('ItemTags',
		'Item','"TagRelations.Tag"',0,'all tags of an item separated by commas',tr_items,1);
...
// papaparse data & add to csv
    function add_to_csv(b_name,APIendpoint,t,){
		let include_header; csvadded == 0 ? include_header = true : include_header = false;
...
// progress bar
	let openbar = window.setInterval(()=>{
		if ((donecount/count) >= 1 && (offset%30000)+100==0 && offset!=0) {
			...
			document.getElementById('progressbar').innerHTML = roundToTwo((donecount/(count+1))*100) + '%';
		}
	}, 200);
// download button
