...
$C = new CLIColors();
echo $C->getColoredString('[START] Deleting tickets.', 'blue') . "\n";
...
try {
    ...
    for ($TicketsCountPage = 1; $TicketsCountPage <= $TicketsCountPages; $TicketsCountPage++) {
        ...
        foreach ($Tickets as $rKey => $rValue) {
            ...
            echo $C->getColoredString($TicketsCount . ". Deleting ticket " . $Tickets[$rKey]['id'] . " ", "green");
            Helper::checkAPISleep($ApiRequestsCount);

            $ApiC->tickets->delete($Tickets[$rKey]['id']);
...
echo $C->getColoredString('[END] Deleting Tickets.', 'blue') . "\n";
